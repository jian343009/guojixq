...
public class Html_channelAndPrice implements IHtml{
	...
	public String getHtml(String content) {
		...
		if(content.isEmpty()){
			for(String cha:new String[]{"华为平台","苹果平台","乐视电视","其它平台","oppo联运"}){
				...
				for(int i=2;i<=21;i++){//单课价格
					int price = data.get("价格").get(i).asInt();
					String msg = data.get("内容").get(i).asString();
					String sid = cha+i;
					内容 +="<tr>\n"
							+ "<td>"+i+"课</td>\n"
							+ "<td><a href='#' data-role='button' onclick=\"udp('"+cha+"','"+i+"');\">"
									+ "<span id=\""	+sid+"p\">"+price+"</span></a></td>\n"//价格输入框
				...
				渠道们 +="<div id=\""+cha+"\" class=\"channels\">\n<table data-role=\"table\""
						+ " data-mode=\"column\" class=\"ui-responsive table-stroke\" border='1'>\n"
					+ "<thead>"
						+ "<tr>"
							+ "<th width=\"45px\">课程</th>\n"
