...
public class CMD203 implements ICMD {
	private final Logger log = Logger.getLogger(CMD203.class.getName());
	...
	public ChannelBuffer getBytes(int cmd, ChannelBuffer data) {
		...
		if(money >0){
			...
			String agent_bill_id = "guojixiangqi"+ServerTimer.getTotalWithS();
			String pay_amt = money+".00";
			String notify_url = "http://guojixqpaycallback.miracle-cn.com:"+HelloServer.httpPort+"/heepay";
			String return_url = "http://www.miracle-cn.com";
			String user_ip = "127_0_0_1";
			String agent_bill_time = ServerTimer.getTotal();
			int goods_num = 1;
			String remark = device.getId()+"-"+lesson+"-"+total+"-"+channel;
...
//					"&is_test=1"+
	        ...
	        buf.writeBytes(Global.getUTF("https://pay.heepay.com/Payment/Index.aspx?"+param));
		}else{
			buf.writeByte(2);
			buf.writeInt(device.getId());
