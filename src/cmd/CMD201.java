...
public class CMD201 implements ICMD {
	...
	public ChannelBuffer getBytes(int cmd, ChannelBuffer data) {
		...
		Global.addRecord(deviceID, imei, "CMD201解锁"+lesson, "");
		...
		String code = "还未购买该课";
		int pow = 1<<lesson;
		if((device.getBuyState() & pow) == pow || BaseData.getContent(BaseData.强制全部解锁).contains("#"+device.getChannel()+device.getVersion()+"#")){
